<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Verify OTP — EN.AR</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Source+Serif+4:opsz,wght@8..60,600;8..60,700&family=Source+Sans+3:wght@400;500;600&display=swap" rel="stylesheet">
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body>
    <x-navbar />
    <main class="block">
        <div class="container auth-box">
            <p class="auth-brand">EN.AR</p>
            <h1 class="auth-title">Verify OTP</h1>
            <p class="lead">OTP verification form will be built here.</p>
            <a href="/login" class="btn btn-outline">Back to log in</a>
        </div>
    </main>
</body>
</html>
